...
(function(){
    ...
    angular.module('hapi-auth')
        .controller('NotesListCtrl', ['$rootScope', '$scope', '$state', 'Note', function($rootScope, $scope, $state, Note){
            $scope.files = [];
            $scope.count = 0;
            $scope.pages = 1;
            $scope._ = _;

            Note.query($state.params.tag || '%', $state.params.page * 1 || 1).then(function(response){
